import { PelangganBankAccount } from './pelanggan.bank_account.entity';
import { PelangganClient } from './pelanggan.client.entity';
import { PelangganClientContact } from './pelanggan.client_contact.entity';
import { PelangganClientSite } from './pelanggan.client_site.entity';
import { PelangganCompanyConfig } from './pelanggan.company_config.entity';
import { PelangganContactClientUse } from './pelanggan.contact_client_use.entity';
import { PelangganContract } from './pelanggan.contract.entity';
import { PelangganContractJenis } from './pelanggan.contract_jenis.entity';
import { PelangganContractSite } from './pelanggan.contract_site.entity';
import { PelangganContractSiteService } from './pelanggan.contract_site_service.entity';
import { PelangganFaktur } from './pelanggan.faktur.entity';
import { PelangganInvoice } from './pelanggan.invoice.entity';
import { PelangganInvoiceItem } from './pelanggan.invoice_item.entity';
import { PelangganKantor } from './pelanggan.kantor.entity';
import { PelangganPayment } from './pelanggan.payment.entity';
import { PelangganPenawaran } from './pelanggan.penawaran.entity';
import { PelangganPenawaranHistory } from './pelanggan.penawaran_history.entity';
import { PelangganPenawaranItem } from './pelanggan.penawaran_item.entity';
import { PelangganPenawaranRevisiLog } from './pelanggan.penawaran_revisi_log.entity';
import { PelangganPenawaranTag } from './pelanggan.penawaran_tag.entity';
import { PelangganPurchaseInvoice } from './pelanggan.purchase_invoice.entity';
import { PelangganPurchaseInvoiceItem } from './pelanggan.purchase_invoice_item.entity';
import { PelangganPurchaseOrder } from './pelanggan.purchase_order.entity';
import { PelangganPurchaseOrderItem } from './pelanggan.purchase_order_item.entity';
import { PelangganSaleShipping } from './pelanggan.sale_shipping.entity';
import { PelangganSaleShippingItem } from './pelanggan.sale_shipping_item.entity';
import { PelangganSalesBarang } from './pelanggan.sales_barang.entity';
import { PelangganSalesPenjualan } from './pelanggan.sales_penjualan.entity';
import { PelangganSalesPenjualanItem } from './pelanggan.sales_penjualan_item.entity';
import { PelangganService } from './pelanggan.service.entity';
import { PelangganTeknisi } from './pelanggan.teknisi.entity';
import { PelangganWorkSchedule } from './pelanggan.work_schedule.entity';
import { PelangganWorkScheduleBap } from './pelanggan.work_schedule_bap.entity';
import { PelangganWorkScheduleBapItem } from './pelanggan.work_schedule_bap_item.entity';
import { PelangganWorkScheduleTeknisi } from './pelanggan.work_schedule_teknisi.entity';

export const PelangganEntities = [
  PelangganBankAccount,
  PelangganClient,
  PelangganClientContact,
  PelangganClientSite,
  PelangganCompanyConfig,
  PelangganContactClientUse,
  PelangganContract,
  PelangganContractJenis,
  PelangganContractSite,
  PelangganContractSiteService,
  PelangganFaktur,
  PelangganInvoice,
  PelangganInvoiceItem,
  PelangganKantor,
  PelangganPayment,
  PelangganPenawaran,
  PelangganPenawaranHistory,
  PelangganPenawaranItem,
  PelangganPenawaranRevisiLog,
  PelangganPenawaranTag,
  PelangganPurchaseInvoice,
  PelangganPurchaseInvoiceItem,
  PelangganPurchaseOrder,
  PelangganPurchaseOrderItem,
  PelangganSaleShipping,
  PelangganSaleShippingItem,
  PelangganSalesBarang,
  PelangganSalesPenjualan,
  PelangganSalesPenjualanItem,
  PelangganService,
  PelangganTeknisi,
  PelangganWorkSchedule,
  PelangganWorkScheduleBap,
  PelangganWorkScheduleBapItem,
  PelangganWorkScheduleTeknisi
];

export * from './pelanggan.bank_account.entity';
export * from './pelanggan.client.entity';
export * from './pelanggan.client_contact.entity';
export * from './pelanggan.client_site.entity';
export * from './pelanggan.company_config.entity';
export * from './pelanggan.contact_client_use.entity';
export * from './pelanggan.contract.entity';
export * from './pelanggan.contract_jenis.entity';
export * from './pelanggan.contract_site.entity';
export * from './pelanggan.contract_site_service.entity';
export * from './pelanggan.faktur.entity';
export * from './pelanggan.invoice.entity';
export * from './pelanggan.invoice_item.entity';
export * from './pelanggan.kantor.entity';
export * from './pelanggan.payment.entity';
export * from './pelanggan.penawaran.entity';
export * from './pelanggan.penawaran_history.entity';
export * from './pelanggan.penawaran_item.entity';
export * from './pelanggan.penawaran_revisi_log.entity';
export * from './pelanggan.penawaran_tag.entity';
export * from './pelanggan.purchase_invoice.entity';
export * from './pelanggan.purchase_invoice_item.entity';
export * from './pelanggan.purchase_order.entity';
export * from './pelanggan.purchase_order_item.entity';
export * from './pelanggan.sale_shipping.entity';
export * from './pelanggan.sale_shipping_item.entity';
export * from './pelanggan.sales_barang.entity';
export * from './pelanggan.sales_penjualan.entity';
export * from './pelanggan.sales_penjualan_item.entity';
export * from './pelanggan.service.entity';
export * from './pelanggan.teknisi.entity';
export * from './pelanggan.work_schedule.entity';
export * from './pelanggan.work_schedule_bap.entity';
export * from './pelanggan.work_schedule_bap_item.entity';
export * from './pelanggan.work_schedule_teknisi.entity';
