...
import { AclUsersReportDto } from 'src/dto/acl/acl.users-report.dto';;

@ApiTags('users_report')
@Controller('users_report')
export class UsersReportController {
    constructor(private readonly service: UsersService) { }

    @Post('list')
    @ApiResponseEntity(AclUsersReportDto, 'list') 
    async findAll( 
    ...
    @Get(':id')
    @ApiResponseEntity(AclUsersReportDto, 'get')
    async findOne(@Param('id') id: string) {
        try {
            ...
            // Jalankan include
            await applySmartInclude([safeResult], filteredIncludes, this.service['repo'].manager);
            return ApiResponseHelper.success(safeResult, 'get');
        } catch (error) {
