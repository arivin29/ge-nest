...
export class UsersController {
    constructor(private readonly service: UsersService, private readonly config: ConfigService) { }

    @Get()
    @ApiResponseEntity(UsersDto, 'list')
    @AutoSwaggerQuery()
    ...
    @Get(':id')
    @ApiResponseEntity(UsersDto, 'get')
    ...
    @ApiResponseEntity(UsersDto, 'post')
    async create(@Body() body: UsersDto) {
        try {

            const secret = this.config.get<string>('jwt.secret');
            const salt = await bcrypt.genSalt(10);
            const saltedPassword = body.password + secret;
    ...
    @Put(':id')
    @ApiBody({ type: UsersDto })
    @ApiResponseEntity(UsersDto, 'put')
    async update(@Param('id') id: string, @Body() body: UsersDto) {
        try {
    ...
    @Delete(':id')
    @ApiResponseEntity(UsersDto, 'delete')
