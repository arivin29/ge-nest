...
import { CompanyConfigService } from '../company_config/company_config.service';
...
import { PelangganCompanyConfigReportDto } from 'src/dto/pelanggan/pelanggan.company_config-report.dto';;

@ApiTags('company_config_report')
@Controller('company_config_report')
export class CompanyConfigReportController {
    constructor(private readonly service: CompanyConfigService) { }

    @Post('list')
    @ApiResponseEntity(PelangganCompanyConfigReportDto, 'list') 
    async findAll( 
    ...
    @Get(':id')
    @ApiResponseEntity(PelangganCompanyConfigReportDto, 'get')
