...
import { WorkScheduleService } from '../work_schedule/work_schedule.service';
...
import { PelangganWorkScheduleReportDto } from 'src/dto/pelanggan/pelanggan.work_schedule-report.dto';;

@ApiTags('work_schedule_report')
@Controller('work_schedule_report')
export class WorkScheduleReportController {
    constructor(private readonly service: WorkScheduleService) { }

    @Post('list')
    @ApiResponseEntity(PelangganWorkScheduleReportDto, 'list')
    async findAll(
        ...
        const parsed: SmartQueryInput = {
            ...
            include: (source as any).include ?? [
                ...
                { name: 'contract_site_service', type: 'single' },
    ...
    @Get(':id')
    @ApiResponseEntity(PelangganWorkScheduleReportDto, 'get')
    async findOne(@Param('id') id: string) {
        try {
            ...
            const allIncludes: SmartQueryInput['include'] = [
                { name: 'contract_site', type: 'single' },
                { name: 'client_site', to: 'contract_site', type: 'single' },
                { name: 'contract_site_service', type: 'single' },
            ...
            // });
            // console.log('filteredIncludes', filteredIncludes)
            // Jalankan include
            await applySmartInclude([result], allIncludes, this.service['repo'].manager);
