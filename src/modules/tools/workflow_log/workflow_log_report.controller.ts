...
import { WorkflowLogService } from '../workflow_log/workflow_log.service';
...
import { ToolsWorkflowLogReportDto } from 'src/dto/tools/tools.workflow_log-report.dto';;

@ApiTags('workflow_log_report')
@Controller('workflow_log_report')
export class WorkflowLogReportController {
    constructor(private readonly service: WorkflowLogService) { }

    @Post('list')
    @ApiResponseEntity(ToolsWorkflowLogReportDto, 'list') 
    async findAll( 
        ...
        const parsed: SmartQueryInput = {
            ...
            include: (source as any).include ?? [
                                                { name: 'log', type: 'single' },
                                                { name: 'data', type: 'single' },
                                                { name: 'step', type: 'single' },
                                                { name: 'users', type: 'single' },
    ...
    @Get(':id')
    @ApiResponseEntity(ToolsWorkflowLogReportDto, 'get')
