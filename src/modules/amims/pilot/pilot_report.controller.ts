...
import { AmimsPilotReportDto } from 'src/dto/amims/amims.pilot-report.dto';;

@ApiTags('pilot_report')
@Controller('pilot_report')
export class PilotReportController {
    constructor(private readonly service: PilotService) { }

    @Post('list')
    @ApiResponseEntity(AmimsPilotReportDto, 'list') 
    async findAll( 
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsPilotReportDto, 'get')
