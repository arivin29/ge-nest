...
import { AmimsWpReportDto } from 'src/dto/amims/amims.wp-report.dto';;

@ApiTags('wp_report')
@Controller('wp_report')
export class WpReportController {
    constructor(private readonly service: WpService) { }

    @Post('list')
    @ApiResponseEntity(AmimsWpReportDto, 'list') 
    async findAll( 
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsWpReportDto, 'get')
