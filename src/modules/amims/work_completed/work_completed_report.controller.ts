...
import { WorkCompletedService } from '../work_completed/work_completed.service';
...
import { AmimsWorkCompletedReportDto } from 'src/dto/amims/amims.work_completed-report.dto';;

@ApiTags('work_completed_report')
@Controller('work_completed_report')
export class WorkCompletedReportController {
    constructor(private readonly service: WorkCompletedService) { }

    @Post('list')
    @ApiResponseEntity(AmimsWorkCompletedReportDto, 'list') 
    async findAll( 
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsWorkCompletedReportDto, 'get')
