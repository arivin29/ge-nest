...
import { MpcWpService } from '../mpc_wp/mpc_wp.service';
...
import { AmimsMpcWpReportDto } from 'src/dto/amims/amims.mpc_wp-report.dto';;

@ApiTags('mpc_wp_report')
@Controller('mpc_wp_report')
export class MpcWpReportController {
    constructor(private readonly service: MpcWpService) { }

    @Post('list')
    @ApiResponseEntity(AmimsMpcWpReportDto, 'list') 
    async findAll( 
        ...
        const parsed: SmartQueryInput = {
            ...
            include: (source as any).include ?? [
                                                { name: 'user_mpc', type: 'single' },
                                                { name: 'user_mm', type: 'single' },
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsMpcWpReportDto, 'get')
