...
import { EngineerPrisItemService } from '../engineer_pris_item/engineer_pris_item.service';
...
import { AmimsEngineerPrisItemReportDto } from 'src/dto/amims/amims.engineer_pris_item-report.dto';;

@ApiTags('engineer_pris_item_report')
@Controller('engineer_pris_item_report')
export class EngineerPrisItemReportController {
    constructor(private readonly service: EngineerPrisItemService) { }

    @Post('list')
    @ApiResponseEntity(AmimsEngineerPrisItemReportDto, 'list') 
    async findAll( 
        ...
        const parsed: SmartQueryInput = {
            ...
            include: (source as any).include ?? [
                                                { name: 'engineer_pris', type: 'single' },
                                                { name: 'part', type: 'single' },
                                                { name: 'part_remove', type: 'single' },
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsEngineerPrisItemReportDto, 'get')
