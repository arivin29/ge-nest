...
import { MpcSelfMaintenanceService } from '../mpc_self_maintenance/mpc_self_maintenance.service';
...
import { AmimsMpcSelfMaintenanceReportDto } from 'src/dto/amims/amims.mpc_self_maintenance-report.dto';;

@ApiTags('mpc_self_maintenance_report')
@Controller('mpc_self_maintenance_report')
export class MpcSelfMaintenanceReportController {
    constructor(private readonly service: MpcSelfMaintenanceService) { }

    @Post('list')
    @ApiResponseEntity(AmimsMpcSelfMaintenanceReportDto, 'list') 
    async findAll( 
        ...
        const parsed: SmartQueryInput = {
            ...
            include: (source as any).include ?? [
                                                { name: 'engineer_rti', type: 'single' },
                                                { name: 'user_maintenance', type: 'single' },
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsMpcSelfMaintenanceReportDto, 'get')
