...
import { PurPrclService } from '../pur_prcl/pur_prcl.service';
...
import { AmimsPurPrclReportDto } from 'src/dto/amims/amims.pur_prcl-report.dto';;

@ApiTags('pur_prcl_report')
@Controller('pur_prcl_report')
export class PurPrclReportController {
    constructor(private readonly service: PurPrclService) { }

    @Post('list')
    @ApiResponseEntity(AmimsPurPrclReportDto, 'list') 
    async findAll( 
        ...
        const parsed: SmartQueryInput = {
            ...
            include: (source as any).include ?? [
                                                { name: 'internal_grn_item', type: 'single' },
                                                { name: 'pur_grn_item', type: 'single' },
                                                { name: 'pur_vendor', type: 'single' },
                                                { name: 'pur_grn', type: 'single' },
                                                { name: 'mpart', type: 'single' },
                                                { name: 'part_condition', type: 'single' },
                                                { name: 'user_pu', type: 'single' },
                                                { name: 'user_qa', type: 'single' },
                                                { name: 'currency', type: 'single' },
                                                { name: 'life_time_limit', type: 'single' },
                                                { name: 'condition_monitoring', type: 'single' },
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsPurPrclReportDto, 'get')
