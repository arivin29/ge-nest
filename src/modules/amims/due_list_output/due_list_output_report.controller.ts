...
import { DueListOutputService } from '../due_list_output/due_list_output.service';
...
import { AmimsDueListOutputReportDto } from 'src/dto/amims/amims.due_list_output-report.dto';;

@ApiTags('due_list_output_report')
@Controller('due_list_output_report')
export class DueListOutputReportController {
    constructor(private readonly service: DueListOutputService) { }

    @Post('list')
    @ApiResponseEntity(AmimsDueListOutputReportDto, 'list') 
    async findAll( 
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsDueListOutputReportDto, 'get')
