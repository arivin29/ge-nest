...
import { PurPoItemService } from '../pur_po_item/pur_po_item.service';
...
import { AmimsPurPoItemReportDto } from 'src/dto/amims/amims.pur_po_item-report.dto';;

@ApiTags('pur_po_item_report')
@Controller('pur_po_item_report')
export class PurPoItemReportController {
    constructor(private readonly service: PurPoItemService) { }

    @Post('list')
    @ApiResponseEntity(AmimsPurPoItemReportDto, 'list') 
    async findAll( 
        ...
        const parsed: SmartQueryInput = {
            ...
            include: (source as any).include ?? [
                                                { name: 'pur_po', type: 'single' },
                                                { name: 'mpart', type: 'single' },
                                                { name: 'pur_pr_item', type: 'single' },
                                                { name: 'engineer_rti', type: 'single' },
                                                { name: 'part', type: 'single' },
                                                { name: 'qa_rpd', type: 'single' },
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsPurPoItemReportDto, 'get')
