...
import { StoreDestroyService } from '../store_destroy/store_destroy.service';
...
import { AmimsStoreDestroyReportDto } from 'src/dto/amims/amims.store_destroy-report.dto';;

@ApiTags('store_destroy_report')
@Controller('store_destroy_report')
export class StoreDestroyReportController {
    constructor(private readonly service: StoreDestroyService) { }

    @Post('list')
    @ApiResponseEntity(AmimsStoreDestroyReportDto, 'list') 
    async findAll( 
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsStoreDestroyReportDto, 'get')
