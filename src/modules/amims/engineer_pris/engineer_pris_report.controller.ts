...
import { EngineerPrisService } from '../engineer_pris/engineer_pris.service';
...
import { AmimsEngineerPrisReportDto } from 'src/dto/amims/amims.engineer_pris-report.dto';;

@ApiTags('engineer_pris_report')
@Controller('engineer_pris_report')
export class EngineerPrisReportController {
    constructor(private readonly service: EngineerPrisService) { }

    @Post('list')
    @ApiResponseEntity(AmimsEngineerPrisReportDto, 'list') 
    async findAll( 
        ...
        const parsed: SmartQueryInput = {
            ...
            include: (source as any).include ?? [
                                                ...
                                                { name: 'position', type: 'single' },
                                                { name: 'aircraft', type: 'single' },
                                                { name: 'user_close', type: 'single' },
                                                { name: 'site', type: 'single' },
                                                { name: 'user_issued', type: 'single' },
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsEngineerPrisReportDto, 'get')
