...
import { TdTypeService } from '../td_type/td_type.service';
...
import { AmimsTdTypeReportDto } from 'src/dto/amims/amims.td_type-report.dto';;

@ApiTags('td_type_report')
@Controller('td_type_report')
export class TdTypeReportController {
    constructor(private readonly service: TdTypeService) { }

    @Post('list')
    @ApiResponseEntity(AmimsTdTypeReportDto, 'list') 
    async findAll( 
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsTdTypeReportDto, 'get')
