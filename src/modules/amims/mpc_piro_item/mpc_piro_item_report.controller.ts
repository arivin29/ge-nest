...
import { MpcPiroItemService } from '../mpc_piro_item/mpc_piro_item.service';
...
import { AmimsMpcPiroItemReportDto } from 'src/dto/amims/amims.mpc_piro_item-report.dto';;

@ApiTags('mpc_piro_item_report')
@Controller('mpc_piro_item_report')
export class MpcPiroItemReportController {
    constructor(private readonly service: MpcPiroItemService) { }

    @Post('list')
    @ApiResponseEntity(AmimsMpcPiroItemReportDto, 'list') 
    async findAll( 
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsMpcPiroItemReportDto, 'get')
