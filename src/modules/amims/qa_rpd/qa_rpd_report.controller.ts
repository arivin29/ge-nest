...
import { QaRpdService } from '../qa_rpd/qa_rpd.service';
...
import { AmimsQaRpdReportDto } from 'src/dto/amims/amims.qa_rpd-report.dto';;

@ApiTags('qa_rpd_report')
@Controller('qa_rpd_report')
export class QaRpdReportController {
    constructor(private readonly service: QaRpdService) { }

    @Post('list')
    @ApiResponseEntity(AmimsQaRpdReportDto, 'list') 
    async findAll( 
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsQaRpdReportDto, 'get')
