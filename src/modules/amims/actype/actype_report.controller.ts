...
import { ActypeService } from '../actype/actype.service';
...
import { AmimsActypeReportDto } from 'src/dto/amims/amims.actype-report.dto';;

@ApiTags('actype_report')
@Controller('actype_report')
export class ActypeReportController {
    constructor(private readonly service: ActypeService) { }

    @Post('list')
    @ApiResponseEntity(AmimsActypeReportDto, 'list') 
    async findAll( 
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsActypeReportDto, 'get')
