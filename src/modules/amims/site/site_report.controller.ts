...
import { AmimsSiteReportDto } from 'src/dto/amims/amims.site-report.dto';;

@ApiTags('site_report')
@Controller('site_report')
export class SiteReportController {
    constructor(private readonly service: SiteService) { }

    @Post('list')
    @ApiResponseEntity(AmimsSiteReportDto, 'list') 
    async findAll( 
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsSiteReportDto, 'get')
