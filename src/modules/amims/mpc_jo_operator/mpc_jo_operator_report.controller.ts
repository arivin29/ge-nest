...
import { MpcJoOperatorService } from '../mpc_jo_operator/mpc_jo_operator.service';
...
import { AmimsMpcJoOperatorReportDto } from 'src/dto/amims/amims.mpc_jo_operator-report.dto';;

@ApiTags('mpc_jo_operator_report')
@Controller('mpc_jo_operator_report')
export class MpcJoOperatorReportController {
    constructor(private readonly service: MpcJoOperatorService) { }

    @Post('list')
    @ApiResponseEntity(AmimsMpcJoOperatorReportDto, 'list') 
    async findAll( 
        ...
        const parsed: SmartQueryInput = {
            ...
            include: (source as any).include ?? [
                                                { name: 'mpc_jo', type: 'single' },
                                                { name: 'operator', type: 'single' },
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsMpcJoOperatorReportDto, 'get')
