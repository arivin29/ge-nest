...
import { PurPoService } from '../pur_po/pur_po.service';
...
import { AmimsPurPoReportDto } from 'src/dto/amims/amims.pur_po-report.dto';;

@ApiTags('pur_po_report')
@Controller('pur_po_report')
export class PurPoReportController {
    constructor(private readonly service: PurPoService) { }

    @Post('list')
    @ApiResponseEntity(AmimsPurPoReportDto, 'list') 
    async findAll( 
        ...
        const parsed: SmartQueryInput = {
            ...
            include: (source as any).include ?? [
                                                ...
                                                { name: 'pur_vendor_contact', type: 'single' },
                                                { name: 'user_manager', type: 'single' },
                                                { name: 'user_director', type: 'single' },
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsPurPoReportDto, 'get')
