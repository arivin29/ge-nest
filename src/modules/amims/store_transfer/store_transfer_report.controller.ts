...
import { StoreTransferService } from '../store_transfer/store_transfer.service';
...
import { AmimsStoreTransferReportDto } from 'src/dto/amims/amims.store_transfer-report.dto';;

@ApiTags('store_transfer_report')
@Controller('store_transfer_report')
export class StoreTransferReportController {
    constructor(private readonly service: StoreTransferService) { }

    @Post('list')
    @ApiResponseEntity(AmimsStoreTransferReportDto, 'list') 
    async findAll( 
        ...
        const parsed: SmartQueryInput = {
            ...
            include: (source as any).include ?? [
                                                { name: 'user_received', type: 'single' },
                                                { name: 'engineer_pmtr', type: 'single' },
                                                { name: 'user_transfer', type: 'single' },
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsStoreTransferReportDto, 'get')
