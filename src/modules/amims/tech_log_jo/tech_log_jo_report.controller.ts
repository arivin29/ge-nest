...
import { TechLogJoService } from '../tech_log_jo/tech_log_jo.service';
...
import { AmimsTechLogJoReportDto } from 'src/dto/amims/amims.tech_log_jo-report.dto';;

@ApiTags('tech_log_jo_report')
@Controller('tech_log_jo_report')
export class TechLogJoReportController {
    constructor(private readonly service: TechLogJoService) { }

    @Post('list')
    @ApiResponseEntity(AmimsTechLogJoReportDto, 'list') 
    async findAll( 
        ...
        const parsed: SmartQueryInput = {
            ...
            include: (source as any).include ?? [
                                                ...
                                                { name: 'tech_log_close', type: 'single' },
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsTechLogJoReportDto, 'get')
