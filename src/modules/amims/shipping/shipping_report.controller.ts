...
import { AmimsShippingReportDto } from 'src/dto/amims/amims.shipping-report.dto';;

@ApiTags('shipping_report')
@Controller('shipping_report')
export class ShippingReportController {
    constructor(private readonly service: ShippingService) { }

    @Post('list')
    @ApiResponseEntity(AmimsShippingReportDto, 'list') 
    async findAll( 
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsShippingReportDto, 'get')
