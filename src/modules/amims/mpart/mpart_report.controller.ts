...
import { MpartService } from '../mpart/mpart.service';
...
import { AmimsMpartReportDto } from 'src/dto/amims/amims.mpart-report.dto';;

@ApiTags('mpart_report')
@Controller('mpart_report')
export class MpartReportController {
    constructor(private readonly service: MpartService) { }

    @Post('list')
    @ApiResponseEntity(AmimsMpartReportDto, 'list') 
    async findAll( 
        ...
        const parsed: SmartQueryInput = {
            ...
            include: (source as any).include ?? [
                                                { name: 'ata', type: 'single' },
                                                { name: 'partid', type: 'single' },
                                                { name: 'group', type: 'single' },
                                                { name: 'part_category', type: 'single' },
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsMpartReportDto, 'get')
