...
import { AmimsBinReportDto } from 'src/dto/amims/amims.bin-report.dto';;

@ApiTags('bin_report')
@Controller('bin_report')
export class BinReportController {
    constructor(private readonly service: BinService) { }

    @Post('list')
    @ApiResponseEntity(AmimsBinReportDto, 'list') 
    async findAll( 
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsBinReportDto, 'get')
