...
import { PurPrItemService } from '../pur_pr_item/pur_pr_item.service';
...
import { AmimsPurPrItemReportDto } from 'src/dto/amims/amims.pur_pr_item-report.dto';;

@ApiTags('pur_pr_item_report')
@Controller('pur_pr_item_report')
export class PurPrItemReportController {
    constructor(private readonly service: PurPrItemService) { }

    @Post('list')
    @ApiResponseEntity(AmimsPurPrItemReportDto, 'list') 
    async findAll( 
        ...
        const parsed: SmartQueryInput = {
            ...
            include: (source as any).include ?? [
                                                { name: 'pur_pr', type: 'single' },
                                                { name: 'mpart', type: 'single' },
                                                { name: 'mpc_piro_item', type: 'single' },
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsPurPrItemReportDto, 'get')
