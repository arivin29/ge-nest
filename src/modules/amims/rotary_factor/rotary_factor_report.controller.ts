...
import { RotaryFactorService } from '../rotary_factor/rotary_factor.service';
...
import { AmimsRotaryFactorReportDto } from 'src/dto/amims/amims.rotary_factor-report.dto';;

@ApiTags('rotary_factor_report')
@Controller('rotary_factor_report')
export class RotaryFactorReportController {
    constructor(private readonly service: RotaryFactorService) { }

    @Post('list')
    @ApiResponseEntity(AmimsRotaryFactorReportDto, 'list') 
    async findAll( 
    ...
    @Get(':id')
    @ApiResponseEntity(AmimsRotaryFactorReportDto, 'get')
